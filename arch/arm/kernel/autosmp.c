...
static int __cpuinit set_enabled (char const* val, struct kernel_param const* kp) {
	int ret;
	unsigned int cpu;
...
#if DEBUG
static ssize_t show_times_hotplugged(struct kobject* a, struct attribute* b, char* buf) {
	ssize_t len = 0;
	int cpu;
